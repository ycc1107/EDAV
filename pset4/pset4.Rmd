...
```{r}
data <- read_tsv('ucd1.txt', col_names = TRUE, n_max=51) %>% as.data.frame() %>% transmute(region=tolower(State), value=Deaths/Population*100) 
#data <- data[complete.cases(data), ] 1020
smap <- state_choropleth(data,title = "Underlying Cause of Death, 1999-2017", legend = "Percent Deaths")
smap
```
